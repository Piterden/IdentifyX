    ...
    Ext.applyIf(config, {
        ...
        viewConfig: {
            ...
            getRowClass: function(rec, ri, p) {
                return rec.data.blocked ? 'identifyx-grid-row-disabled' : '';
            }
        },
        paging: true,
        remoteSort: true,
...
Ext.extend(IdentifyX.grid.Data, MODx.grid.Grid, {
    ...
    getMenu: function(grid, row) {
        ...
        if (cs.length > 1) {
            ...
            if (true) {
                ...
                m.push({
                    text: '<i class="icon icon-check-circle-o"></i>&nbsp;' + _('identifyx_users_enable'),
                    handler: function() {
                        this.unblockUser(cs);
                    }
                });
            ...
            if (true) {
                m.push({
                    text: '<i class="icon icon-times-circle-o error"></i>&nbsp;' + _('identifyx_user_disable'),
                    handler: function() {
                        this.blockUser(cs);
                    }
                });
                ...
                m.push({
                    text: '<i class="icon icon-check-circle-o"></i>&nbsp;' + _('identifyx_user_enable'),
                    handler: function() {
                        this.unblockUser(cs);
                    }
                });
    ...
    getColumns: function(config) {
        return [{
            ...
            xtype: 'booleancolumn',
            header: _('identifyx_user_blocked_col'),
            dataIndex: 'blocked',
            sortable: false,
            trueText: '<button class="btn btn-sm btn-danger"><i class="icon icon-times-circle-o"></i>',
            falseText: '<button class="btn btn-sm btn-success"><i class="icon icon-check-circle-o"></i>',
            editable: true,
            editor: {
                xtype: 'trigger',
            },
    ...
    getTopBar: function(config) {
        return [
            ...
            '->', {
                xtype: 'identifyx-combo-user',
                emptyText: _('identifyx_select_user'),
                width: 200,
                listeners: {
                    select: {
                        fn: this.filterByUser,
                        scope: this
                    },
                },
            }, {
