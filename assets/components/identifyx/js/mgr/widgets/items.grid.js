IdentifyX.grid.Items = function (config) {
	config = config || {};
	if (!config.id) {
	...
	IdentifyX.grid.Items.superclass.constructor.call(this, config);
...
Ext.extend(IdentifyX.grid.Items, MODx.grid.Grid, {
	windows: {},
	...
	unblockUser: function (act, btn, e) {
		var ids = this._getSelectedIds();
		if (!ids.length) {
			return false;
		}
	...
	getFields: function (config) {
		return ['id', 'user_id', 'fingerprint', 'votes', 'blocked', 'actions'];
	},
...
Ext.reg('identifyx-grid-data', IdentifyX.grid.Items);
